...
def test_macropdf():
    # given an image that has two parts of a multi-bar code datum
    image = PIL.open("tests/macropdf_part1and2.png")
    # and another image that the thirst part (e.g. 2nd page_
    image2 = PIL.open("tests/macropdf_part3.png")

    info_list = []
    # when we decode first page
    decoder = PDF417Decoder(image)
    barcode_count = decoder.decode()
    assert barcode_count == 2
    info_list.extend(decoder.barcodes_info)

    # and the 2nd page
    decoder = PDF417Decoder(image2)
    barcode_count = decoder.decode()
    assert barcode_count == 1
    info_list.extend(decoder.barcodes_info)

    assert len(info_list) == 3
    assert PDF417Decoder.assemble_data(info_list) == b'One Two Three'
